import type { CombatPokemonStats, CombatState, Move, CombatAction } from "../type/combat"
...
const WILD_POKEMON = [
    263, 265, 270, 273, 276, 278, 280, 283, 285, 290,
    293, 296, 300, 304, 307, 309, 311, 312, 315, 318,
    320, 322, 325, 328, 331, 333, 335, 336, 339, 341
]
...
export async function buildCombatPokemon(pokemonId: number, level: number): Promise<CombatPokemonStats> {
...
export async function buildMoves(pokemonId: number): Promise<Move[]> {
    ...
    for (const type of data.types.map((t: any) => t.type.name)) {
...
function calculateDamage(attacker: CombatPokemonStats, defender: CombatPokemonStats, move: Move): number {
    const effectiveness = isEffective(move.type, defender.types)
    const damage = Math.floor((move.power * attacker.attack) / defender.defense) * effectiveness
...
export function playTurn(state: CombatState, action: CombatAction): void {
    if (action.type === "catch") {
        ...
        const catchRate = hpRatio < 0.25 ? 0.55 : hpRatio < 0.5 ? 0.35 : 0.2
        ...
        if (Math.random() < catchRate) {
            ...
            const oppMove = state.opponentMoves[random(0, state.opponentMoves.length - 1)]
            const damage = calculateDamage(state.opponent, state.player, oppMove)
            state.player.hp -= damage
            state.messages.push(`${state.opponent.name} utilise ${oppMove.name} (-${damage} HP)`)
    ...
    if (action.type === "flee") {
        if (Math.random() < 0.5) {
            ...
            const damage = calculateDamage(state.opponent, state.player, oppMove)
            state.player.hp -= damage
            state.messages.push(`${state.opponent.name} attaque avec ${oppMove.name} (-${damage} HP)`)
    ...
    const playerDamage = calculateDamage(state.player, state.opponent, playerMove)
    const oppDamage = calculateDamage(state.opponent, state.player, oppMove)

    state.messages.push(`${state.player.name} utilise ${playerMove.name} (-${playerDamage} HP)`)
    ...
    state.messages.push(`${state.opponent.name} utilise ${oppMove.name} (-${oppDamage} HP)`)
...
export async function createCombatVsWild(playerPokemonId: number, wildPokemonId: number): Promise<CombatState> {
...
export async function createCombat(playerPokemonId: number): Promise<CombatState> {
...
export function calculateRewards(state: CombatState): { moneyGained: number } {
