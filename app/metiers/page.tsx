...
export default function Metiers() {
  ...
  const loadData = async () => {
    setLoading(true);
    const res = await getJobDataAction();
    setLoading(false);
    if (res.success) {
      setJobData(res.data);
  ...
  const selectJob = async (jobId: string) => {
    const res = await chooseJobAction(jobId);
    if (res.success) {
      await loadData();
      setView("main");
    } else {
  ...
  const startMission = async () => {
    const res = await startMissionAction();
    if (res.success) {
      await loadData();
    } else {
      setError(res.error);
  ...
  const completeMission = async (missionId: string) => {
    const res = await completeMissionAction(missionId);
    if (res.success) {
      await loadData();
    } else {
      setError(res.error);
        ...
        <div className="card-body">
          ...
          <div className="flex gap-6 mt-4 text-sm">
            <div className="badge badge-outline gap-2">
              ...
              <span>
                {Math.floor(
                  job!.baseReward * (1 + jobData.level * 0.1)
                )}{" "}
                ₽
              </span>
            </div>
...
function ActiveMission({
  ...
  const progress = Math.min(
    100,
    (Date.now() - mission.createdAt.getTime()) / (job.duration * 1000) * 100
  );
