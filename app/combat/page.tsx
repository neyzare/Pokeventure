...
export default function Combat() {
  ...
  useEffect(() => {
    getCombatHistoryAction().then((res) => {
  ...
  const startSelection = async () => {
    setLoading(true);
    const res = await getMyPokemonAction();
    setLoading(false);
    if (res.success) {
  ...
  const selectPokemon = async (pokemonId: number) => {
    setLoading(true);
    const res = await startCombatAction(pokemonId);
    setLoading(false);
    if (res.success) {
        ...
        {history.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {history.map((c) => (
              ...
              >
                <img
                  src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${c.opponentId}.png`}
                  alt=""
                  className="w-10 h-10"
                />
                ...
                <div className="flex gap-3">
                  {c.moneyGained > 0 && (
                    <span className="text-xs">+{c.moneyGained} ₽</span>
                  )}
                  <span
...
function BattleView({
  ...
  const attack = async (moveIndex: number) => {
    ...
    setLoading(true);
    const res = await playTurnAction({ type: "attack", moveIndex });
    setLoading(false);
  ...
  const flee = async () => {
    if (loading || combat.status !== "ongoing") return;
    setLoading(true);
    const res = await playTurnAction({ type: "flee" });
    setLoading(false);
    if (res.success) setCombat(res.combat);
  };
        ...
        <div className="bg-slate-50 p-6">
          <div className="flex justify-between items-start mb-8">
            ...
            <div className="flex flex-col items-end gap-2">
              <HpBar
                name={combat.opponent.name}
                level={combat.opponent.level}
        ...
        <div className="border-t border-slate-200 px-6 py-4 h-28 overflow-y-auto">
          {combat.messages.length === 0 ? (
            <p className="text-sm text-slate-500">
              Un{" "}
              <span className="capitalize font-medium">{combat.opponent.name}</span>{" "}
              sauvage apparaît !
            </p>
          ) : (
            combat.messages.map((msg, i) => (
          ...
          {isOver ? (
            <div className="text-center py-2">
              ...
              <button
                type="button"
                onClick={onEnd}
                className="rounded-xl border bg-blue-900 px-6 py-2.5 text-sm font-semibold text-white hover:bg-blue-950"
              >
                Retour
              </button>
            </div>
