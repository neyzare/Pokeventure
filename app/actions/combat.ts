...
export async function playTurnAction(
  action: CombatAction
): Promise<
  | { success: true; combat: CombatState }
  | { success: false; error: string }
> {
  ...
  if (combat.status !== "ongoing")
    return { success: false, error: "Combat terminé" };
...
export async function getMyPokemonAction(): Promise<
...
export async function getCombatHistoryAction(): Promise<
