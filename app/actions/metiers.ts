...
export async function getJobDataAction(): Promise<
...
export async function chooseJobAction(jobId: string): Promise<
  | { success: true }
  | { success: false; error: string }
> {
  const user = await getAuthenticatedUser();
...
export async function startMissionAction(): Promise<
...
> {
  ...
  try {
    ...
    if (!userRecord || !userRecord.job)
      return { success: false, error: "Aucun métier choisi" };
...
export async function completeMissionAction(missionId: string): Promise<
...
> {
  ...
  try {
    ...
    if (mission.userId !== user.id)
      return { success: false, error: "Mission non autorisée" };
    ...
    if (timeElapsed < requiredTime) {
      return {
        success: false,
        error: `Attendre ${Math.ceil((requiredTime - timeElapsed) / 1000)}s`,
      };
    }
