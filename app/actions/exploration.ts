...
// Note: In-memory store - for production with serverless, consider Redis
...
> {
  ...
  if (pokedex.length === 0)
    return { success: false, error: "Vous n'avez pas encore de Pokémon" };
...
export async function startWildEncounterAction(
...
export async function playWildTurnAction(
  action: CombatAction
): Promise<
  | { success: true; combat: CombatState; caught: boolean }
  | { success: false; error: string }
> {
  const user = await getAuthenticatedUser();
  if (!user) return { success: false, error: "Non connecté" };
  ...
  if (combat.status !== "ongoing")
    return { success: false, error: "Combat déjà terminé" };

  playTurn(combat, action);

  // playTurn mutates combat.status - TypeScript doesn't track this
  const status = combat.status as CombatState["status"];
  if (status === "caught") {
