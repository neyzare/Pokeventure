...
async function fetchDetails(id: number): Promise<PokemonDetails> {
  ...
  const types = (json.types as any[])
    .sort((a, b) => a.slot - b.slot)
    .map((t) => t.type.name as string);

  const bst = (json.stats as any[]).reduce((sum, s) => sum + (s.base_stat as number), 0);
...
export function usePokemonDetails(id: number) {
  const [details, setDetails] = React.useState<PokemonDetails | null>(() => cache.get(id) ?? null);
